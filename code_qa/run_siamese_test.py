            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        ...
                        if results['acc'] >= best_results['acc']:
                            best_results = results
                            # for key in best_results.keys():
                            #     logger.info("  Best %s = %s", key, str(round(best_results[key],4)))

                            # save
                            checkpoint_prefix = 'checkpoint-best-aver'  # TODO checkpoint-best-mrr
...
def evaluate(args, model, tokenizer,eval_when_training=False):
    ...
    if eval_dataset is None:
        ...
        eval_dataset = TextDataset(tokenizer, args, eval_data_path)
    ...
    eval_loss = torch.tensor(eval_loss / nb_eval_steps)  # 其实就是cross entropy loss
...
def test(args, model, tokenizer):
    if not args.test_predictions_output:
        args.test_predictions_output = os.path.join(args.output_dir, 'predictions.txt')
    ...
    test_data_path = os.path.join(args.data_dir, args.test_data_file)
...
def main():
    ...
    if args.augment:
        model = ModelContraOnline(model, config, tokenizer, args)
    else:
